...
// The prefix is what makes a 75hard token recognisable on sight.
var TokenScheme = goapi.NewScheme("75h_")
...
type createTokenRequest struct {
	Name string `json:"name"`
	// Scopes is read, or read and write. Anything unrecognised normalises to
	// read — the safe reading of an ambiguous request cannot change data.
	Scopes []string `json:"scopes"`
	// ExpiresInDays of 0 means it never expires.
	ExpiresInDays int `json:"expires_in_days"`
}

// HandleCreateToken issues a new personal API token.
//
...
func (s *Server) HandleCreateToken(w http.ResponseWriter, r *http.Request) {
	...
	// A token issued by a token would let a read credential quietly escalate
	// itself into a write one, so this is reachable only with a real login.
	if TokenAuth(ctx) {
	...
	s.log.Info("api token created",
		zap.Int64("user", userID), zap.Int64("token", id), zap.String("scopes", scopes.String()))

	respondJSON(w, http.StatusCreated, map[string]any{
		"token": token,
		// Shown once. No endpoint can return this again.
		"secret": cred.Plaintext,
		// Everything needed to start using it, including where the OpenAPI
		// document lives.
...
func (s *Server) HandleListTokens(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, name, prefix, scopes, last_used_at, expires_at, created_at
		 ...
		 ORDER BY created_at DESC`, UserID(ctx))
	if err != nil {
...
//
// Revoking rather than deleting keeps the row, so a token that turns up in a
// log later can still be identified as one that existed and was withdrawn.
func (s *Server) HandleRevokeToken(w http.ResponseWriter, r *http.Request) {
	...
	res, err := s.db.ExecContext(ctx, `
		UPDATE api_tokens SET revoked_at = CURRENT_TIMESTAMP
		 WHERE id = ? AND user_id = ? AND revoked_at IS NULL`, id, UserID(ctx))
	if err != nil {
...
// TokenScopes returns the scopes of the API token used, if any.
func TokenScopes(ctx context.Context) goapi.Scopes {
	scopes, _ := ctx.Value(TokenScopeKey).(goapi.Scopes)
	return scopes
}

// tokenStore adapts the api_tokens table to go-api's Store.
//
// It is generic over the subject, which here is the user's integer id;
// folioworth supplies a UUID through the same interface.
...
func (t tokenStore) Lookup(ctx context.Context, hash string) (goapi.Record, int64, error) {
	...
	err := t.s.db.QueryRowContext(ctx, `
		SELECT tok.user_id, tok.scopes, tok.revoked_at IS NULL, tok.expires_at,
	...
	// A token outliving its account must not keep working.
	if deleted {
...
// Touch records last use. Called after the decision, so a failure here is
// logged rather than allowed to fail the request.
...
// tokenAuthenticator builds the verifier for incoming API tokens.
func (s *Server) tokenAuthenticator() *goapi.Authenticator[int64] {
	auth := goapi.NewAuthenticator[int64](TokenScheme, tokenStore{s})
	auth.OnTouchError = func(err error) {
