...
//
// Claims is deliberately field- and tag-identical to go-login's Claims so that
// tokens minted by the OAuth flow validate here without translation.
package auth
...
// BcryptCost is the work factor for password hashing. 12 is ~250ms per hash;
// tests lower it via SetBcryptCost so they don't spend their life hashing.
var BcryptCost = 12
...
// with HMAC so a caller cannot downgrade us to "alg": "none".
