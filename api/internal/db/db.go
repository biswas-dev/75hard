...
// taskai uses. No migration library.
package db
...
// DB wraps *sql.DB with the handful of helpers the handlers need.
type DB struct {
	*sql.DB
}

// Open connects to the SQLite database at path, creating the parent directory
// if needed, and applies the pragmas that make SQLite behave under a web
// server: WAL for concurrent readers, a busy timeout so a concurrent writer
// waits instead of returning SQLITE_BUSY, and enforced foreign keys.
func Open(path string) (*DB, error) {
	...
	// second query while iterating the rows of a first — with a pool of one,
	// that second query waits forever for a connection the first still holds.
	// Write contention is handled by busy_timeout, not by starving the pool.
