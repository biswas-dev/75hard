...
//
// go-login owns no schema of its own; this is the whole contract between the
// library and the app.
...
func (s *OAuthStore) FindUserByProviderID(ctx context.Context, provider, providerUserID string) (*gologin.User, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil // not found is not an error in this interface
	}
	if err != nil {
...
// FindUserByEmail looks up an existing account by email so an OAuth sign-in
// can be linked to it rather than creating a duplicate.
...
// GetUserAuthProvider reports how the account signs in: "password", "google"
// or "github".
...
// CreateOAuthUser provisions an account from a provider profile.
//
// inviteCode is ignored: this instance has open registration rather than
// invite-gating, and the parameter exists only to satisfy the interface.
func (s *OAuthStore) CreateOAuthUser(ctx context.Context, info gologin.ProviderUserInfo, provider, inviteCode string) (*gologin.User, error) {
	...
	// password_hash is NOT NULL, and an OAuth account has no password, so it
	// gets an unusable random hash that no input can ever match.
...
// LinkOAuthProvider attaches a provider identity to an existing account, so
// signing in with Google after registering with a password reaches the same
...
func randomPlaceholderHash() (string, error) {
	...
	// MinCost: this hash exists to be unmatchable, not to be checked, so
	// there is no reason to pay for a real work factor.
